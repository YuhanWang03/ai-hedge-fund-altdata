  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100 text-slate-800">
      ...
      <header className="h-12 shrink-0 bg-white border-b border-slate-200 flex items-center px-4 gap-3">
        ...
        <span className="text-xs text-slate-400">操作面板 + 聊天</span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={saveToken}
            className="text-xs rounded bg-slate-800 text-white px-2 py-1">保存</button>
          <button onClick={() => setRefreshKey((k) => k + 1)}
            className="text-xs rounded border border-slate-300 px-2 py-1">刷新</button>
        </div>
      </header>
