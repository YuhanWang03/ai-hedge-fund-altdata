...
export default function Home() {
  ...
  useEffect(() => { const updateClock = () => setCurrentTime(formatClock(new Date())); updateClock(); const timer = window.setInterval(updateClock, 1000); return () => window.clearInterval(timer) }, []);
  useEffect(() => {
...
const STATUS_LABELS: Record<string,string> = { COMPLETED: '已完成', PARTIAL: '部分完成', PARTIAL_DATA: '部分数据', PARTIAL_ERROR: '部分异常', RUNNING: '运行中', PENDING: '等待中', FAILED: '失败', PASSED: '已通过', FULL: '完整支持', STANDARD: '标准支持', LIMITED: '有限支持', INSUFFICIENT: '证据不足', HEALTHY: '正常', CACHED: '缓存', UNAVAILABLE: '不可用', AVAILABLE: '可用', AVAILABLE_WHEN_PROVIDER_RETURNS: '数据源返回时可用' };
...
function MetricRows({ rows }: { rows: [string, unknown, string?][] }) { return <div className="engine-metric-rows">{rows.map(([label,value,kind]) => <div key={label}><span>{translateResearchText(label)}</span><strong>{translateResearchText(formatResearchValue(value,kind))}</strong></div>)}</div> }
...
function SecDepthPanel({ result }: { result: StockResearchResult }) {
  ...
  return <section className="surface engine-card"><div className="surface-header"><div><h2>SEC 深度发现</h2><span>10-K / 10-Q / 8-K 正文结构化结果</span></div><StatusPill value={result.modules.sec.status}/></div><div className="sec-depth-grid"><div><strong>风险因素变化</strong>{changes.length ? changes.slice(0,10).map((row,index) => <article key={`${String(row.title)}-${index}`}><em>{translateResearchText(row.change_type)}</em><b>{translateResearchText(row.title)}</b><p>{String(row.text || '').slice(0,240)}</p></article>) : <p className="insufficient-note">没有足够的连续文件形成可靠差异。</p>}</div><div><strong>结构化发现</strong>{findings.length ? findings.slice(0,10).map((row,index) => <article key={`${String(row.accession_number)}-${index}`}><em>{String(row.filing_type)}</em><b>{translateResearchText(row.title)}</b><p>{String(row.summary || '').slice(0,240)}</p></article>) : <p className="insufficient-note">正文暂未成功解析。</p>}</div></div></section>;
}
