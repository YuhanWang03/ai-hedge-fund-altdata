...
import { apiJson, askAgentV2, authHeaders, getAgentV2Job, type AgentV2Evidence, type AgentV2Job, type AgentV2Response, type AgentV2SubAgent, type AgentV2TraceStep } from './lib/api';
...
function SubAgentTrace({ label, rounds, elapsedMs, stop, calls, trace, intraday }: { label: string; rounds: number; elapsedMs: number; stop: string; calls: Record<string, number | null | undefined>; trace: AgentV2TraceStep[]; intraday?: boolean }) {
  ...
  return <details className="agent-trace"><summary>{summary}</summary><ol>{trace.map((step, index) => <li key={`${step.round}-${index}`}><span>第 {step.round} 轮</span><code>{step.action}</code>{step.detail && <em>{step.detail}</em>}<small>{step.ms} ms</small></li>)}{trace.length === 0 && <li><em>没有记录到模型轮次</em></li>}</ol></details>;
}
      ...
      <aside className="chat-panel" aria-label="AI 投资助理">
        ...
        <div className="message-list" ref={messageListRef} aria-live="polite">
          {messages.map(message => <div key={message.id} className={`message ${message.role}`}>
            ...
            {message.agent && message.agent.capabilities.length > 0 && <details className="agent-detail"><summary>执行工具（{message.agent.capabilities.length}）{message.agent.subAgents.length > 0 && ` · 子智能体 ${message.agent.subAgents.length}`}</summary><div className="capability-list">{message.agent.capabilities.map((capability, index) => <code key={`${capability}-${index}`}>{capability}</code>)}</div>{message.agent.subAgents.map((agent, index) => <div key={`${agent.capability}-${index}`} className="sub-agent"><SubAgentTrace label={`${agent.label} ${agent.subject}`} rounds={agent.rounds} elapsedMs={agent.elapsed_ms} stop={agent.stop_reason} calls={agent.calls} trace={agent.trace} intraday={agent.intraday}/>{agent.nested.map((nested, nestedIndex) => <div key={nestedIndex} className="sub-agent nested"><SubAgentTrace label={nested.label} rounds={nested.rounds} elapsedMs={nested.elapsed_ms} stop={nested.stop_reason} calls={nested.calls} trace={nested.trace}/></div>)}</div>)}</details>}
