    ...
    <div className="lab-tool lab-tool-fill">
      ...
      <section className="surface lab-result lab-result-clamp">
        {error ? <ErrorBox text={error}/> : !result ? <Empty glyph="↗" title="等待回测" text={meta.empty}/> : <>
          ...
          {result.strategy === 'committee' && (result.notes?.periods?.length || 0) > 0 && <div className="lab-table-wrap"><div className="lab-subhead">每期投票（{result.notes!.periods!.length} 个换仓日）</div><table className="lab-table"><thead><tr><th>换仓日</th><th>数据截至</th><th>股票</th><th>投票 / 弃权</th><th>多 / 空 / 中</th><th>共识</th><th>一致度</th><th>入选</th></tr></thead><tbody>{result.notes!.periods!.flatMap(pd => [...pd.verdicts.map(v => <tr key={`${pd.signal_date}-${v.ticker}`} className={v.picked ? 'picked' : ''}><td>{pd.signal_date}</td><td>{pd.as_of}</td><td><strong>{v.ticker}</strong></td><td>{v.voters} / {v.abstained}{v.gaps ? ` · ${v.gaps} 处缺数据` : ''}</td><td>{v.bullish} / {v.bearish} / {v.neutral}</td><td className={v.consensus >= 0 ? 'positive' : 'negative'}>{num(v.consensus)}</td><td>{pctAbs(v.agreement)}</td><td>{v.picked ? '✓' : v.voters === 0 ? '全部弃权' : '—'}</td></tr>), ...pd.missing.map(t => <tr key={`${pd.signal_date}-${t}-x`}><td>{pd.signal_date}</td><td>{pd.as_of}</td><td><strong>{t}</strong></td><td colSpan={5}>取数失败</td></tr>)])}</tbody></table></div>}
