...
function CommitteeTool({ result, setResult, handoff, clearHandoff, onHand, watchlist, refreshWatchlist, ask }: ToolProps & { result?: CommitteeResult; setResult: (r?: CommitteeResult) => void; handoff: Handoff | null; clearHandoff: () => void; onHand: (t: string[], from: string, to: LabTool) => void }) {
  ...
  const [topN, setTopN] = useState('15'); const [maxWeight, setMaxWeight] = useState('15');
  ...
  const run = async () => {
    setBusy(true); setError('');
    try {
      const body: Record<string, unknown> = { source, personas: allSelected ? undefined : selected, max_weight: (Number(maxWeight) || 15) / 100, ...(source === 'tickers' ? { tickers: parseTickers(tickers) } : {}), ...(source === 'screening' ? { top_n: Number(topN) || 15 } : {}) };
