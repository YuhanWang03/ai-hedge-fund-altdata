    ...
    <div className="lab-tool lab-tool-fill">
      ...
      <section className="surface lab-result lab-result-clamp">
        {error ? <ErrorBox text={error}/> : !result ? <Empty glyph="∿" title="等待计算" text="对每次财报公布，用市场模型剔除大盘影响后累计 [0,+1]、[0,+5]、[0,+20] 日的异常收益，并给出 t 检验和 bootstrap 置信区间。"/> : <>
          ...
          {result.events.length > 0 && <div className="lab-table-wrap"><div className="lab-subhead">逐事件</div><table className="lab-table"><thead><tr><th>股票</th><th>日期</th><th>类型</th><th>EPS</th><th>CAR [0,1]</th><th>CAR [0,5]</th><th>CAR [0,20]</th><th>β</th></tr></thead><tbody>{result.events.slice(0, 40).map((e, i) => <tr key={i}><td><strong>{e.ticker}</strong></td><td>{e.event_date}</td><td>{e.source_type}</td><td className={e.eps_surprise === 'BEAT' ? 'positive' : e.eps_surprise === 'MISS' ? 'negative' : ''}><strong>{e.eps_surprise || '—'}</strong></td><td className={(e.car_0_1 || 0) >= 0 ? 'positive' : 'negative'}>{pct(e.car_0_1, 2)}</td><td className={(e.car_0_5 || 0) >= 0 ? 'positive' : 'negative'}>{pct(e.car_0_5, 2)}</td><td className={(e.car_0_20 || 0) >= 0 ? 'positive' : 'negative'}>{pct(e.car_0_20, 2)}</td><td>{num(e.market_model.beta)}</td></tr>)}</tbody></table></div>}
          </div>
