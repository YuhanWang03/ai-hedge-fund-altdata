...
export function CostPage({ report, loading, error, refresh }: { report: CostReport | null; loading: boolean; error: string; refresh: () => Promise<void> }) {
  ...
  return <div className="cost-page unified-costs">
    ...
    <div className="cost-summary-grid">
      {([['今日已核价估算', 'today_amount'], ['本月已核价估算', 'month_amount'], ['累计已核价估算', 'total_amount']] as const).map(([label, field]) => <section className="surface" key={label}><span>{label}</span>{report ? report.currencies.map(c => <strong className="currency-total" key={c.currency}>{money(c[field], c.currency)}</strong>) : <strong>—</strong>}<small>人民币／美元分开统计，不换算 · 美东时间</small></section>)}
