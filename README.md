...
**Agent V2** ([`v2/agent_v2/`](./v2/agent_v2/README.md)): the current conversational layer — a deterministic planner keyed on a model intent, bounded sub-agents on one tool loop, a synthesizer whose every figure must cite fetched evidence and pass a verifier plus a model judge, clarifying questions, evidence-shaped follow-ups, and a quality loop that scores every change. On the development set (each question asked twice) the hardening loop took the pass rate from **76% to 92%** on the original 40 questions (86% on the final 43, after three investigator cases were added), fallbacks from 3 to 1, and the token equivalent per question down 8%; a 13-question hold-out set never read while iterating scores 77%, the three lab experiments 3/3; 211 unit tests and a 39-case offline evaluation gate every push.
