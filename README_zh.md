...
**Agent V2**（[`v2/agent_v2/`](./v2/agent_v2/README.md)）：现在的对话层——按模型意图驱动的确定性规划器、跑在同一个工具循环上的有界子智能体、每个数字都必须引用已抓取证据并通过校验器和模型评审的合成器、低置信度反问、基于上一轮证据的追问，以及给每次改动打分的质量评测环。开发集（每题两次）上，这轮加固把原 40 题的通过率从 **76% 提到 92%**（加入三道调查题后 43 题为 86%），回退从 3 次降到 1 次，每题 token 当量降 8%；迭代中从不看的 13 题留出集 77%，三道实验题 3/3；211 个单测和 39 条离线评测在每次 push 上把关。
