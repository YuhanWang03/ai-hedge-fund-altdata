...
一个生产级的另类数据情报平台，以 Telegram bot 作为交付界面。**六个盘后批处理 agent** 持续监控美股市场，**两个财报 cron** 自动追踪 watchlist + 持仓的 D-3/D-1/D-0 提醒和发布后总结，**两个组合风险 cron** 推送每日集中度 / P&L / 回撤快照 + 周度复盘，**两个 SEC 监控 cron** 每日扫描 8-K 重大事件 + Form 4 内部人交易（含同日 ≥3 distinct insiders 集群检测），**一个盘中 Streamer 服务**实时扫描异动并触发用户价格提醒，**二十一个自然语言意图**处理交互式查询，**五重防幻觉机制**保证每一条推送都有可追溯的源头。
...
**Scheduler（调度器）** —— 十四个 cron 任务，覆盖基本面筛选、异动检测、产业链横向扩展、机构 13F 跟踪、周度回填、ETF 每日持仓抓取、财报日历提醒、财报发布后总结、组合风险快照、周度复盘、SEC 8-K 重大事件、SEC Form 4 内部人交易、P2 日汇总、archive 清扫。每个任务进程隔离，单个崩溃不会污染其他。
...
**Telegram Bot** —— 24/7 长轮询机器人，支持 21 个 slash 命令、3 个 watchlist 命令，以及一个含 21 种意图的自然语言分类器。通过 chat ID 过滤实现单用户授权。
...
│  NL 分类器      · 21 个意图 · DeepSeek T=0 · 严格枚举            │
└──────────────────────────────────────────────────────────────────┘
...
机器人的自然语言层把任意中文 / 英文文本归类到 **21 个固定意图**之一，分类器是 DeepSeek temperature=0 + JSON 输出 + 白名单枚举校验。**任何不在白名单的输出都 fallback 到 `unknown`**，保证行为有界。
...
- 10-Q parser + MD&A 关键变化 diff（季度报告，频率低但信息密度高）
- 每周 insider digest cron（聚合 noise_summary 里的 A/M/F/G/C 用季度趋势 view）
- per-position 周表现归因 → Phase 2.5 已经记录，继续延后
...
- **272 个 sandbox 单元测试** —— Phase 0/1/2/3 累计：Phase 1 (21 earnings byte-equal + 17 earnings priority + 9 earnings pipeline + 10 earnings cron integration) + Phase 2 (9 portfolio formatter byte-equal + 19 portfolio priority + 5 priority floor + 10 portfolio pipeline edges + 13 portfolio cron integration + 20 portfolio smoke) + Phase 3 (21 SEC smoke + 21 SEC priority + 15 SEC byte-equal + 8 HTML safety + 20 cron integration + 11 bot responder) + 通用 (9 archive migration + 23 intent classify + 18 priority + 13 observability + 余下分布)

### 进行中 / TODO

- **Phase 3.5 (optional)** —— 10-Q parser + MD&A 关键变化 diff / 每周 insider digest cron（聚合 A/M/F/G/C noise_summary）/ per-position 周表现归因（Phase 2 Stage 7 留尾）
- **Phase 4 · Macro Agent（已 Stage 0 ack，等 `FRED_API_KEY` 上 VPS）** —— FOMC / CPI / PCE / NFP 通过 FRED + Tavily 推送，含动态生成的 macro calendar
...
仓库外壳和原始教育版 `app/` 目录来自 [virattt/ai-hedge-fund](https://github.com/virattt/ai-hedge-fund)——一个开源 AI hedge fund 概念项目。`v2/` 目录——也就是本 README 描述的整个另类数据 agent 系统（六个盘后 agent + 两个财报 cron + 两个组合风险 cron + 两个 SEC 监控 cron、盘中 Streamer、含 21 个 NL intent 的 Telegram bot、5 层防幻觉机制、推送优先级系统、所有 SQLite + ChromaDB 存储、所有 systemd 部署脚手架）——是在那个基础上**完全从零搭建的独立项目**。
