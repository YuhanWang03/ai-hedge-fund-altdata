...
**Agent 层**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **89 条标注评测集、每条采样 3 次**下实测：通过率 **53% → 94%**，成本为完整循环的 **55%**，267 次运行中**归属误报 1 例、稳定失败 0 条**。另有 197 个测试。
...
| | 单跳（现状） | **routed**（生产形态） | 完整 agent 循环 |
|---|---|---|---|
| **通过率** | 53% | **94%** | 93% |
| 工具召回 / 事实召回 | 70% / 63% | 98% / 96% | 99% / 98% |
| 溯源率 | 100% | 99% | 98% |
| **归属误报** | — | **1 / 267** | 2 / 267 |
| token / 例 | 0 | **5,270** | 9,604 |
| **每通过 token** | — | **5,629** | 10,298 |
| 稳定失败 | — | — | **0 条** |
...
**路由在两个轴上都赢：通过率高 2pp，成本只有它的 54%。** 单跳在它设计要解决的问题上是满分，
...
**Agent 层：另有 197 个测试**（`python3 -m v2.agent.run_tests`），既不需要 pytest 也不需要任何第三方包 —— 循环、路由、两个检查和评测 harness 在导入期只依赖标准库；模型被脚本化，所以「工具失败能不能恢复」是一条断言而不是一段轶事。
