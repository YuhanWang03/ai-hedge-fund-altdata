...
**Agent V2**（[`v2/agent_v2/`](./v2/agent_v2/README.md)）：现在的对话层——按模型意图驱动的确定性规划器、跑在同一个工具循环上的有界子智能体、每个数字都必须引用已抓取证据并通过校验器和模型评审的合成器、低置信度反问、基于上一轮证据的追问，以及给每次改动打分的质量评测环。40 题开发集（每题两次）上，这轮加固把通过率从 **76% 提到 92%**，回退从 3 次降到 1 次，每题 token 当量降 11%；189 个单测和 39 条离线评测在每次 push 上把关。
