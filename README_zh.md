...
**Agent 层**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **83 条标注评测集、每条采样 3 次**下实测：通过率 **52% → 95%**，成本为完整循环的 **61%**。另有 159 个测试。
...
83 条标注 query，9 个类别，每条采样 3 次（249 次运行）。工具层是录制观测，
因此唯一的变量是模型。

| | 单跳（现状） | **routed**（生产形态） | 完整 agent 循环 |
|---|---|---|---|
| **通过率** | 52% | **95%** | **95%** |
| 工具召回 / 事实召回 | 71% / 63% | 99% / 97% | 100% / 99% |
| 溯源率 | 100% | 99% | 98% |
...
**路由与完整循环打平，成本只有它的 61%。** 单跳在它设计要解决的问题上是满分，
...
└── agent/                          # 模型驱动循环 + 路由 + 会话 + 溯源/归属检查 + 83 条评测集
```
...
**Agent 层：另有 159 个测试**（`python3 -m v2.agent.run_tests`），既不需要 pytest 也不需要任何第三方包 —— 循环、路由、两个检查和评测 harness 在导入期只依赖标准库；模型被脚本化，所以「工具失败能不能恢复」是一条断言而不是一段轶事。
