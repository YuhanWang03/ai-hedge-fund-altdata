...
**Agent 层**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **89 条标注评测集、每条采样 3 次**下实测：通过率 **53% → 96%**，成本为完整循环的 **57%**；线上配置（8 次调用）两次全量 **92–95%**、成本 **45%**。267 次运行中归属误报 0–1 例；校验器的第二种错误率（重写丢掉初稿已有的事实）修复后 0 / 267。另有 215 个测试。
