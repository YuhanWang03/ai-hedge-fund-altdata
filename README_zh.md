...
**路由是对的，线上那个更紧的配置也是对的。** routed 以 57% 的成本拿到 96%；production 再收紧到 8 次调用，两次全量 92–95%、成本 45% —— 掉的在 `ranking` 和 `causal`。同一份代码两次全量在 production 上差 3pp，这套评测的分辨率约 ±2pp。 单跳在它设计要解决的问题上是满分，
