...
**Agent V2**（[`v2/agent_v2/`](./v2/agent_v2/README_zh.md)）：现在的对话层——按模型意图驱动的确定性规划器、跑在同一个工具循环上的有界子智能体、每个数字都必须引用已抓取证据并通过校验器和模型评审的合成器、低置信度反问、基于上一轮证据的追问，以及给每次改动打分的质量评测环。开发集（每题两次）上，这轮加固把原 40 题的通过率从 **76% 提到 92%**（加入三道调查题后 43 题为 86%），回退从 3 次降到 1 次，每题 token 当量降 8%；迭代中从不看的 13 题留出集 77%，三道实验题 3/3；211 个单测和 39 条离线评测在每次 push 上把关。

**Agent 层（V1）**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **89 条标注评测集、每条采样 3 次**下实测：通过率 **53% → 96%**，成本为完整循环的 **57%**；线上配置（8 次调用）两次全量 **92–95%**、成本 **45%**。267 次运行中归属误报 0–1 例；校验器的第二种错误率（重写丢掉初稿已有的事实）修复后 0 / 267。另有 215 个测试。
...
上面所有内容都是**代码驱动**的管道：`v2/bot/intent.py` 把一句话映射到 24 个 label
之一，`commands.py` 调用一个 responder，一次。对定时推送而言这是正确的设计，
但它答不了「我持仓里哪只最危险」—— 这个判断需要权重、财报临近度、内部人交易，
而它们分散在三个不同的工具里。

[`v2/agent/`](./v2/agent/README.md) 在同一批 responder 之上加了一个**模型驱动**的
循环（调什么、调几次、按什么顺序，由模型当场决定），外加一个按问题决定走哪条路的
路由层。全程只做加法：`cmd_nl` 增加 32 行受 flag 控制的代码，`v2/bot` 其余一行未动。

### 用测量说话，不用断言说话

89 条标注 query，10 个类别，每条采样 3 次（267 次运行）。工具层是录制观测，
因此唯一的变量是模型。

| | 单跳（现状） | **routed**（20 次） | **production**（8 次，线上） | 完整 agent 循环 |
|---|---|---|---|---|
| **通过率** | 53% | **96%** | 92%（上次全量 95%） | 97% |
| 工具召回 / 事实召回 | 70% / 63% | 98% / 98% | 98% / 97% | 99% / 99% |
| 溯源率 | 100% | 99% | 97% | 99% |
...
**路由是对的，线上那个更紧的配置也是对的。** routed 以 57% 的成本拿到 96%；production 再收紧到 8 次调用，两次全量 92–95%、成本 45% —— 掉的在 `ranking` 和 `causal`。同一份代码两次全量在 production 上差 3pp，这套评测的分辨率约 ±2pp。 在 **15 条来自 Telegram 实测、从未用来调过任何东西的留出问题**上：完整循环 **100%**，routed **84%**，路由准确率 **87%**——漏的全是主题词表不认识的双主题问题。过拟合的是手写路由，不是模型循环。 单跳在它设计要解决的问题上是满分，
在答案必须跨工具的地方几乎为零；路由几乎白拿了循环的全部上行，同时避开了它在
单卡问题上的下行 —— 那里循环会过度探索，然后在自己的溯源检查上摔倒。

### 循环加了什么，又被什么约束

预算与强制收尾轮 · 错误即观测（工具失败是喂给模型的信息，不是运行的终点） ·
重复调用抑制 · 近因加权上下文压缩 · **溯源**：答案里每个数字都必须能追到某条观测，
推导出来的数字**只有在答案写出算式、且算式输入自身可溯源时**才算数 ·
**归属**：数字还必须属于它被写在旁边的那个主体 —— 这是溯源检查看不见的
「数字是真的、公司是错的」那类失败。
...
1. **校验器不奖励的规则，模型不会遵守。** 连续三轮要求模型「写出算式」毫无效果；
   把检查改成**接受写出的算式**之后，行为立刻就变了。
2. **单次运行的失败清单大部分是噪声。** 同一配置跑两轮分数相同，但 25 条不同的
   失败只有 9 条重合，因此 harness 对每条采样三次，把稳定失败与抖动分开。
3. **指标会对自己的对象撒谎。** 「18% 的回答无法溯源」听起来像模型在编造，
   拆开看 71% 是检查自身的缺陷 —— 四舍五入、单位换算、SEC 条款号。

逐轮明细、机制说明与已知限制：[`v2/agent/README.md`](./v2/agent/README.md)。
...
└── agent/                          # 模型驱动循环 + 路由 + 会话 + 溯源/归属检查 + 89 条评测集
```
...
**Agent 层：另有 215 个测试**（`python3 -m v2.agent.run_tests`），既不需要 pytest 也不需要任何第三方包 —— 循环、路由、两个检查和评测 harness 在导入期只依赖标准库；模型被脚本化，所以「工具失败能不能恢复」是一条断言而不是一段轶事。
