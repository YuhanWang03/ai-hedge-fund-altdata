...
**Agent 层**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **89 条标注评测集、每条采样 3 次**下实测：通过率 **53% → 95%**，成本为完整循环的 **54%**；线上配置（8 次调用）**93%**、成本 **42%**。267 次运行中归属误报 0–1 例。另有 205 个测试。
...
|---|---|---|---|---|
| **通过率** | 53% | **95%** | 93% | 96% |
| 工具召回 / 事实召回 | 70% / 63% | 98% / 97% | 98% / 97% | 99% / 99% |
| 溯源率 | 100% | 100% | 99% | 99% |
| **归属误报** | — | **0 / 267** | 1 / 267 | 1 / 267 |
| token / 例 | 0 | 5,249 | **3,941** | 9,710 |
| **每通过 token** | — | 5,517 | **4,226** | 10,127 |
...
**路由是对的，线上那个更紧的配置也是对的。** routed 以 54% 的成本拿到 95%；production 再收紧到 8 次调用，93%、成本 42% —— 掉的 2pp 全在 `ranking` 一类。 单跳在它设计要解决的问题上是满分，
...
**Agent 层：另有 205 个测试**（`python3 -m v2.agent.run_tests`），既不需要 pytest 也不需要任何第三方包 —— 循环、路由、两个检查和评测 harness 在导入期只依赖标准库；模型被脚本化，所以「工具失败能不能恢复」是一条断言而不是一段轶事。
