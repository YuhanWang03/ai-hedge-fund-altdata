...
**Agent 层**（[`v2/agent/`](./v2/agent/README.md)）：在同一批 24 个 responder 之上的模型驱动工具调用循环，外加一个按问题判断「多步规划值不值这个钱」的路由层。在 **89 条标注评测集、每条采样 3 次**下实测：通过率 **53% → 96%**，成本为完整循环的 **56%**；线上配置（8 次调用）**95%**、成本 **44%**。267 次运行中归属误报 0 例；校验器的第二种错误率（重写丢掉初稿已有的事实）实测 0–1 / 267。另有 214 个测试。
...
|---|---|---|---|---|
| **通过率** | 53% | **96%** | 95% | 96% |
| 工具召回 / 事实召回 | 70% / 63% | 99% / 98% | 98% / 97% | 99% / 99% |
| 溯源率 | 100% | 100% | 99% | 99% |
| **归属误报** | — | **0 / 267** | 0 / 267 | 0 / 267 |
| 重写丢事实 | — | 1 / 267 | **0 / 267** | 1 / 267 |
| token / 例 | 0 | 5,021 | **3,935** | 8,919 |
| **每通过 token** | — | 5,216 | **4,137** | 9,266 |
...
**路由是对的，线上那个更紧的配置也是对的。** routed 以 56% 的成本拿到 96%；production 再收紧到 8 次调用，95%、成本 44% —— 掉的 1pp 在 `ranking` 和 `dead_end`。 单跳在它设计要解决的问题上是满分，
...
**Agent 层：另有 214 个测试**（`python3 -m v2.agent.run_tests`），既不需要 pytest 也不需要任何第三方包 —— 循环、路由、两个检查和评测 harness 在导入期只依赖标准库；模型被脚本化，所以「工具失败能不能恢复」是一条断言而不是一段轶事。
