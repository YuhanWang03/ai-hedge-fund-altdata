...
一个生产级的另类数据情报平台，以 Telegram bot 作为交付界面。**六个盘后批处理 agent** 持续监控美股市场，**两个财报 cron** 自动追踪 watchlist + 持仓的 D-3/D-1/D-0 提醒和发布后总结，**两个组合风险 cron** 推送每日集中度 / P&L / 回撤快照 + 周度复盘，**两个 SEC 监控 cron** 每日扫描 8-K 重大事件 + Form 4 内部人交易（含同日 ≥3 distinct insiders 集群检测），**一个 SEC 周度内部人摘要 cron** 每周五 19:15 ET 聚合本周 ⑫ 推送 → 单卡周报，**一个 ARK 调仓告警 cron** 每个交易日 08:30 ET pre-market 检测 ARK 4 fund 显著调仓（new/liquidated/±20%）+ 多 fund 协同检测，**四个 Macro Agent cron** 推送每日 VIX / yields snapshot + CPI/PCE/NFP/GDP/PPI 单日 release + FOMC statement diff 与 SEP dot plot + 周度 macro recap，**一个盘中 Streamer 服务**实时扫描异动并触发用户价格提醒，**二十三个自然语言意图**处理交互式查询，**五重防幻觉机制**保证每一条推送都有可追溯的源头。
...
**Scheduler（调度器）** —— 二十一个 cron 任务，覆盖基本面筛选、异动检测、产业链横向扩展、机构 13F 跟踪、周度回填、ETF 每日持仓抓取、ARK 调仓告警（⑬ Mon-Fri 08:30 ET pre-market）、财报日历提醒、财报发布后总结（含 10-Q MD&A 关键变化 diff）、组合风险快照、组合持仓快照（⑨b 静默后台）、周度复盘、SEC 8-K 重大事件、SEC Form 4 内部人交易、SEC 周度内部人摘要（⑫b Fri 19:15 ET）、Macro 日终 snapshot、Macro release scanner、Macro Initial Claims、Macro 周报、P2 日汇总、archive 清扫。每个任务进程隔离，单个崩溃不会污染其他。
...
│  08:30 ET 周一-周五  ⑬ ARK 调仓告警        → Telegram 推送       │
...
ARK Invest 是 Cathie Wood 主动管理的 ETF 家族（ARKK / ARKW / ARKG / ARKF），与传统 ETF 不同——他们**每个交易日公开当日 holdings**，透明度极高。这意味着 Cathie 的 conviction trades 比 13F 季度披露提前 **45 天看到**：新建仓 / 清仓 / 显著调仓都是 same-day 信号。

价值：
- 突然清仓某 ticker = strong negative signal
- 新建仓 +0.5% 起 = strong positive signal（尤其 small-cap）
- 多 fund 同向调仓 = 部门级 conviction（Cathie + co-PM 共识）

### 数据源 — 复用 ⑤ ETF 基础设施 (Stage 0 audit 决定)

Stage 0 audit 发现 v2/etf/ 模块自 Phase 0 起就在跑 ARK CSV 抓取 + 日 diff + SQLite 持久化（⑤ ETF Daily Snapshot 17:00 ET cron）。Phase 5a 复用整套基础设施，**只新增 alerts 分类层 + 卡片 formatter**，零 schema 改动、零 client/detector/tracker 改动。

| Cron | 时间 | 行为 |
|---|---|---|
| ⑤ ETF Daily Snapshot | Mon-Fri **17:00 ET** | 拉 ARK 4 fund CSV → 写 `etf.db.snapshots` 表（archive-only, 无 Telegram 推送） |
| ⑬ ARK Alerts | Mon-Fri **08:30 ET** pre-market | 读 `etf.db` 昨日 baseline + 今日 CSV → diff → 显著调仓 → Telegram 推送 |

### ⑬ ARK Alerts 触发阈值 + Priority Ladder

`SUPPORTED_FUNDS = ["ARKK", "ARKW", "ARKG", "ARKF"]` (ARKQ + ARKX 由 ARK 在 assets.ark-funds.com 已 deprecate，无 CSV 暴露)。

4 类 action + 阈值（`v2/etf/alerts.py` 唯一 source of truth，byte-equal pin 死）：
...
`v2/etf/alerts.py:_mark_multi_fund` 在 per-fund 分类完成后做 cross-fund 聚合：按 `(ticker, direction)` 分组（direction = `"buy"` if action ∈ {new_position, increase} else `"sell"`），≥2 funds 同向同 ticker → 所有匹配 alert mark `is_multi_fund=True`。**反向同 ticker（一买一卖）显式 NOT 协同**——这是不同 fund manager 的独立判断，不是 conviction signal，regression 钉死。

### 卡片 UX (Stage 3 polished)

4 action 各自模板，emoji + 中文标签：

```
🟢 ARK 新建仓 · NVDA          # new_position
🔴 ARK 清仓 · IRBT            # liquidated
📈 ARK 增持 · TSLA            # increase
📉 ARK 减持 · COIN            # decrease
```

加 2 个可选 banner（顺序：multi-fund 优先 → user-universe 次之 → divider）：

```
🚨 多 Fund 协同（详见总览）   # 优先放（P0 关键因子 visual priority）
🟢 持仓股 / 关注列表          # 后放
━━━━━━━━━━━━━━━━━━━━
```

总览卡（每日 1 张，append 在所有 individual 卡之后）含：
- 触发 alerts 总数 + 多 fund 计数 + 涉及持仓 / 关注计数
- 行动分布（**buys cluster** 新建仓+增持 → **sells cluster** 清仓+减持，认知分组）
- 涉及 user universe ticker 列表（sorted）
- 多 fund 协同 ticker 与 fund 详情
- **本日扫描 funds: (N/M ARK funds)** 分数（partial-failure 透明度，e.g. ARKG 503 → "(3/4 ARK funds)" + warnings 行）

### 输出示例

**Multi-fund 协同 + 持仓股 P0 卡（base 65 + held +10 + multi_fund +15 = 90）：**
...
- **Stage 0 audit 节省 87% 代码 / 31% 工时**：spec'd 新建 v2/ark/ ~1500 行 → 复用 v2/etf/ ~250 行 alerts 层 + formatter；audit 是 senior 级判断的最佳例证
- **Multi-fund 协同 NOT 反向同 ticker** regression test 钉死（test_classify_multi_fund_opposite_directions_not_coordinated）
- **funds_attempted backward-compat field** + `_coverage_str` 在 caller 不传 attempted 时 fall back 到 legacy "(N 个)" 显示，旧测试零回归
...
- **byte-equal 跨 surface**：⑬ cron 推送 == `format_ark_alert(reconstructed_alert)` byte-equal pin（Phase 3 / 3.5 cross-surface identity convention）

### 测试覆盖（Phase 5a 新增）

- **16 个 alerts smoke**（`v2/etf/test_alerts.py`：threshold ladder × 4 action + user_universe + multi-fund 3 种 + opposite-direction NOT coordinated + dataclass immutability + edge cases）
- **11 个 byte-equal pin + HTML safety + shim identity**（`v2/etf/test_ark_alert_cards_byte_equal.py`：5 alert templates + 3 summary states + 2 HTML escape + 1 shim identity）
- **8 个 cron integration**（`dashboard/backend/tests/test_ark_cron_integration.py`：normal day / quiet day / multi-fund P0 / held boost / per-fund CSV failure / first-deploy / responder framing / byte-equal cron-vs-formatter）

总计 35 个 Phase 5a 新增测试。Stage 4 cron integration **明确 mirror Phase 3.5 silent-ship 防御 pattern**——assert `priority_reasons` trail 含 literal `multi_fund_coordination` / `held_or_watchlist_ark` 字符串，end-to-end 测试是 priority wire-through bug 的唯一防御。

### 数字 sweep（Phase 5a 完成）

- **20 → 21 cron jobs**（+⑬）
- **439 → 474 tests**（+35 net；Phase 5a 全部 sandbox-runnable）
- **0 new external deps**
- **7 → 8 architecture shims**（+ `format_ark_*` 走 `v2/etf/_ark_alert_cards` → `v2/reporting/_ark_alert_formatters` → `formatters.py` → `__init__.py` 4 层 pattern）
...
- **Phase 5a · ⑬ ARK 调仓告警** —— 详见上一节；含 1 新 alerts 分类层 (`v2/etf/alerts.py`) + 2 新 formatter (`format_ark_alert` / `format_ark_summary`) + 4 priority adjustments (held_or_watchlist_ark / multi_fund_coordination / large_new_position / large_liquidation) + scheduler 20→21 (⑬ Mon-Fri 08:30 ET pre-market) + 35 个新测试 (16 smoke + 11 byte-equal/HTML/shim + 8 cron integration)。Stage 0 audit 发现 v2/etf/ 自 Phase 0 起就在跑 ARK CSV 抓取 + day diff → scope pivot 节省 83% 代码 / 31% 工时 (1500 行 → 250 行)。零 schema 改动、零 client/detector/tracker 改动、零外部依赖
...
- **474 个 sandbox 单元测试** —— Phase 0-4 + 4.5-mini + 2.5 完整版 + 3.5 + 5a 累计：Phase 1 (21 earnings byte-equal + 17 earnings priority + 9 earnings pipeline + 10 earnings cron integration) + Phase 2 (12 portfolio formatter byte-equal + 19 portfolio priority + 5 priority floor + 10 portfolio pipeline edges + 17 portfolio cron integration + 20 portfolio smoke) + Phase 2.5 完整版 (20 snapshot/attribution/drawdown smoke + 6 ⑨b cron integration) + Phase 3 (21 SEC smoke + 21 SEC priority + 15 SEC byte-equal + 8 HTML safety + 20 cron integration + 11 bot responder) + Phase 3.5 (20 Stage 1 smoke + 5 Stage 3 byte-equal + 10 Stage 4 cron integration) + Phase 4 (40 macro smoke + 16 macro priority ladder + 16 macro byte-equal + 9 HTML safety + 9 bot responder + 22 cron integration) + Phase 4.5-mini (12 price_source smoke) + Phase 5a (16 ARK alerts smoke + 11 byte-equal/HTML/shim + 8 cron integration) + 通用 (9 archive migration + 33 intent classify + 18 priority + 13 observability + 余下分布)
...
- **Phase 5b · Market Regime detection** —— defer 到 06-17 ⑮ FOMC + SEP 实战之后，用真实 macro 数据校准 regime 阈值（避免 overfit 历史窗口）
...
> **下一个 Phase 启动条件**：上述 deferred Phase 满足各自 Trigger 后启动。当前处于 production observation 阶段，focus 是 validate Phase 0-5a 在真实数据下的稳定性。

---

## 🛣️ 未来计划（待数据齐全 / Future Work）

以下 Phase 已规划但 **deferred**，等待真实 production 数据积累 / 特定事件触发后启动。**不在当前 sprint scope**。

> **命名澄清** —— 历史上有两个"4.5"系列：
> - **Phase 4.5-mini** ✅ 已 ship — FD → yfinance daily prices migration（commit `b36d5e3` 系列）
> - **Phase 4.5** ⏳ deferred — FOMC transcript + Historical analog mode（本节内容）

### ⏳ Phase 4.5 — FOMC Transcript + Historical Analog Mode

**目标**：
- **Task A**：FOMC ⑮b transcript +6h follow-up 卡 —— Powell 记者会结束后 6 小时从 Bloomberg / Reuters Q&A 摘要抓取，补 statement diff 之外的语调信号（hawkish phrase count / dovish phrase count）
- **Task B**：Historical analog mode —— LLM Layer 4 防御正式落地，LLM 引用"过去 5 次类似 σ surprise 的 SPX 5-day return"作为预测尺度，避免 narrative-only 评论

**为什么 deferred**：
- 需要 ⑮ FOMC + SEP 至少跑过 1-2 次（2026-06-17 + 2026-09-16）才有真实信号 baseline
- Layer 3（Tavily aggregate + Python diff）需在真实数据下验证后再叠加 Layer 4
- Historical analog 需要 backfill SPX + 历史 release dates 3-5 年 ≈ 250 entries，沉默成本不低

**启动条件**：
- 06-17 FOMC + SEP 已实战
- Phase 4 ⑮⑰ 跑过 ≥ 2 个完整月份
- 用户对 Layer 1+2 defense 在生产数据下有反馈

**工时**：1.5-2 天 · **风险**：中（Tavily Q&A 质量 + SPX 历史数据完整性）

### ⏳ Phase 5b — Market Regime Detection

**目标**：
- 综合 VIX + 10Y-2Y spread + market breadth + sentiment 判断市场 regime
- 4 个 regimes: `risk_on` / `risk_off` / `transition` / `extreme_vol`
- 新 ⑭b cron Mon-Fri 16:35 ET（紧跟 ⑭ 16:30），regime 切换 → P0 告警
- 卡片含：当前 regime + 切换历史 (≤ 5) + 各 regime 下持仓表现（跟 ⑨b positions_snapshot 集成做 Brinson-style attribution by regime）

**为什么 deferred**：
- Regime detection 高 overfit 风险 —— 简化阈值（"VIX > 25 = risk_off"）和 hindsight bias 不可分
- Phase 4 ⑰ 周报 + ⑮ release 跑 1+ 月后才有真实 macro 数据可校准切换阈值
- backtest 漂亮但 live 不灵是 regime 模型的经典坑

**启动条件**：
- Phase 4 全套（⑭⑮⑯⑰）实战 ≥ 4 周
- Phase 4.5（above）已 ship 验证 Layer 3/4 防御稳定
- 用户对 macro 信号 已建立判断尺度

**工时**：2-3 天 · **风险**：高（regime detection 容易过拟合）

### ⏳ Phase 6 — News 重构 + 3-tier Universe

**目标**：
- **Task A**：News 层重构 —— 集中 Tavily query 入口 + per-ticker cache + NER ticker mapping，消除多 agent 同日重复 query 同一 ticker 的浪费
- **Task B**：3-tier Universe ——
  - Tier 1 核心（held + 高 conviction watchlist）：daily 全套 cron
  - Tier 2 watchlist（一般关注）：daily lightweight + weekly 深度
  - Tier 3 广义观察（行业 / 题材池）：weekly batch only

**为什么 deferred**：
- 当前 universe 仅 ~10 ticker，3-tier 设计无实际价值（over-engineering）
- News 重构是 cost optimization；当前 Tavily ~500 query/月、LLM ~$1/月，都不是瓶颈
- 重复 query 问题在 small universe 下 marginal（≤ 5% 浪费率）

**启动条件**（任一）：
- 用户 universe 扩大到 ≥ 30 ticker（held + watchlist 合计）
- 或 LLM cost 月度超过 $50（current ~$5-10 范围）
- 或观察到 ≥ 5% 推送是 Tier 3-4 news 噪音

**工时**：4 天 · **风险**：中（NER 准确度 + cache invalidation 永远是难点）

---

## 致谢

仓库外壳和原始教育版 `app/` 目录来自 [virattt/ai-hedge-fund](https://github.com/virattt/ai-hedge-fund)——一个开源 AI hedge fund 概念项目。`v2/` 目录——也就是本 README 描述的整个另类数据 agent 系统（六个盘后 agent + 两个财报 cron + 两个组合风险 cron + 三个 SEC 监控 cron（含 ⑫b 周度内部人摘要）+ 一个 ARK 调仓告警 cron（⑬ pre-market）+ 四个 Macro Agent cron、盘中 Streamer、含 23 个 NL intent 的 Telegram bot、5 层防幻觉机制、推送优先级系统、所有 SQLite + ChromaDB 存储、所有 systemd 部署脚手架）——是在那个基础上**完全从零搭建的独立项目**。
