...
from v2.agent_v2.eval.subagent_ledger import aggregate, ledger_path, read_rows, render, usage_by_source


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Aggregate the sub-agent run ledger and their token usage.")
    ...
    if args.json:
        print(json.dumps({"agents": summary, "usage": usage, "rows": len(rows)}, ensure_ascii=False, indent=2))
    else:
