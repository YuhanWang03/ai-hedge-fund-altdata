...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Agent V1→V2 benchmark · {'holdout' if args.holdout else 'dev'} set · {len(cases)} cases · modes={', '.join(modes)}")
    gaps = gap_summary(cases)
    if gaps:
        print("capability gaps: " + "; ".join(f"{tool} → {len(ids)} case(s)" for tool, ids in gaps.items()))
    print()
    reports = run_benchmark(modes, holdout=args.holdout, repeat=args.repeat)
