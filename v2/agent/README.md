...
| **归属误报** | — | **0 / 267** | 2 / 267 |
| 路由准确率 | — | **98%** | — |
| token / 例 | 0 | **5,493** | 10,045 |
| **每通过 token** | — | **5,867** | 10,903 |
| 稳定失败 | — | — | 2 条 |
...
**结论：routed 在两个轴上都赢 —— 通过率高 2pp，成本只有 54%。** 单跳在它设计要
解决的问题上是满分，在需要跨工具组合的地方（multi_hop 29%、compound 0%、
recovery 0%）几乎为零；路由拿走了 agent 的全部上行，同时避开了它在单卡问题上的
下行（causal 差 17pp、checker_stress 差 28pp）。

这条结论回答了一个很自然的产品提议：「把所有自然语言提问都交给 agent，斜杠命令
才给卡片」。**不要做** —— 那等于花 1.9 倍的钱换 2pp 的通过率下降，而且会让最简单
的查询（`我的持仓`）变成最不可靠的一类。
...
- **超预算率 24%**：agent 经常调用远超必要的工具数（c07 用过 21 次、38k token，
  上限 10）。这不判失败（成本不是正确性），但说明循环仍偏向「探索」而非「决策」，
  而线上每一次多余的 `explain_move` 都是真钱（见上一条）。
- **两条稳定失败，都是工具选择而非表达**：`c08`「我上周亏的钱这周补回来了吗」
  漏调 `pnl_view`（要比两个周期，它只查了一个）；`k06`「我组合里半导体和软件各占
  多少」漏调 `risk_view`，改用 `portfolio_view` 自己算，算出来的行业占比无法溯源。
  两条都以 0/3 复现，是当前最值得动的东西。
