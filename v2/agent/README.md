...
4. **prompt 能纠正「选错工具」，纠正不了「不知道收敛」。** 第 10 轮的对照实验：
   给描述补一个模型无法自己发现的事实（这张卡没有行业列），一条稳定失败当场
   消失；给同一批描述补一条程序性提示（成员要逐个查），完全无效 —— 因为那条
   失败的病根是不知道**什么时候停**，而不是不知道**从哪开始**。
...
- **超预算率 20%（agent）/ 10%（routed）**，且**已知描述改动碰不到它**：
