...
**结论：路由是对的，线上那个更紧的配置也是对的。** 单跳在它设计要解决的问题上
是满分，在需要跨工具组合的地方（multi_hop 29%、compound 0%、recovery 0%）几乎
为零；路由拿走了 agent 几乎全部的上行（差 1pp），成本只有 54%。**production 再把
调用上限从 20 收到 8，又便宜 23%，代价是 2pp —— 而那 2pp 全部落在 `ranking`
一类**（87% → 73%），其余九类都在一条之内。
