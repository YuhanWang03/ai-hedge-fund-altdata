...
- **98% 的路由准确率是开发集分数**：规则是对着这 89 条调出来的，真实泛化能力需要
  没见过的 query。
  **留出集已建，还没跑**：`eval/holdout.py`，15 条，全部来自 Telegram 实测时真实发过的
  问题（A–F 组和用户自己打的），标注时不看路由信号表；与开发集逐字重复的 7 条被
  一条测试剔除，写操作和多轮省略各 2 条因评测不覆盖而排除。预演路由：x10（木头姐
  重叠）和 x11（CPI+NFP 影响）会被留在快路径，这两条当时就知道、没改，正是留出集
  要量的东西。跑法 `EVAL_SET=holdout EVAL_REPEAT=3 poetry run python v2/agent/run_eval.py`，
  跑一次记数字，**不为了让某条通过而改代码**。
