...
- **超预算率 20%（agent）/ 10%（routed）**，且**已经两次证实描述改动碰不到它**
  （第 10 轮的程序性提示、第 11 轮的价目表，见上节）：
  m09、d05、m03 稳定跑到 19–21 次工具调用，上限是 10–12。上一节的对照实验说明
  这不是「不知道该查什么」，是「不知道什么时候够了」—— 循环仍偏向「探索」而非
  「决策」。要动得改终止条件（让模型看见预算余量、给同一工具的扇出设上限），
  而线上每一次多余的 `explain_move` 都是真钱（见上一条）。这是当前最大的一块。
- **稳定失败 0 条，抖动 12 条**：抖动按定义没法凭一轮定位，3 次采样还不够。
