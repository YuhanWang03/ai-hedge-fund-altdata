...
| routed（开发集） | 96% | 5,455 | **87%**（存档 84%） | **2,990** |
| production（开发集） | 92–95% | 4,404 | 84%（存档 81%） | 2,924 |
| agent（开发集） | 97% | 9,396 | 81%（存档 78%） | 6,306 |
...
- **通过率差 9–16pp，token 少 45%。** gpt-4.1-mini 每例 2.0 次调用对 deepseek 的 3.4，
  超预算 0–1% 对 11–16%。它更省，也更早停：多数失败是「工具漏调」——问「哪只离财报
  最近」它调 8 次 `earnings_view` 自己比，不调那张日历卡；问「AAPL 财报怎么样」它只讲
  上一次，不给下次日期。
- **存档分和重打分差 3pp，那 3pp 是答案键的。** 「CRWD 什么时候财报，8-K 有什么」
  两张卡都调了，答案里写的是「2026 年 9 月 6 日」，标签只认 `2026-09-06` 和 `9-06`。
  deepseek-chat 照抄卡片上的 ISO 格式，十五轮标注没有一轮暴露过这一点——评测一直在
  测日期的拼写。`normalise` 现在把年月日、ISO、`M-DD` 归一到同一个形式，然后用
  `python -m v2.agent.eval.rescore` 拿存档的答案重打：三档各 +7～8 次，全是 p03、p06、
  v05 的日期；留出集 +0。这是一次放宽，而且是看了第二个模型的答案之后做的，所以写
  清楚：它只认等价的写法，不针对任何一条用例；deepseek 的分数不受影响（它写 ISO），
  但它那份 JSON 已被覆盖，无法重打验证。
- **在 gpt-4.1-mini 上，完整循环反而不如路由**（81% 对 87%，留出集 76% 对 71% 又反过来）。
