...
| 14 | 6 条抖动，温度已是 0 | 记录每次运行的路径和初稿，按分叉点分类。**6 条里 3 条是 3 次采样的噪声**；剩下 3 条是三个不同的 bug，两个在校验器里。误报轴看不见其中最重的那个：警告打在**初稿**上，重写把事实一起扔了，记成了模型的「事实缺失」 | d06/r09/m07 各 **10/10**；全量 routed **96%** / production **95%** |
...
下一条指令无效**。样本是 3 次打回，只能说方向对了，全量的读数在下一次重跑里。
新增 `partial_rewrite` 标记（重写丢了初稿过半的溯源数字），把「只回了一段」和
「整段改坏」分开计。
...
- **重写指令的改动只在 p04 上量过**：10 次里 3 次打回、丢事实 1 → 0，样本小。全量
  的「重写丢事实」读数要等下一次 89 × 3 重跑。
