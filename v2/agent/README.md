...
两条是同一个形状：`SPY←-14(实为 TSLA)`——「同期 SPY +0.40% → 相对强度
-3.14pp」里的 SPY 是**基准**，不是主语。这也正是整个系列**第一个**误报
（「相对 SMH 逆势 -8.30pp」）的形状，绕了十二个修复又回来了。修法是识别
**比较句式**（同期／相对／跑输／vs）而不是把 SPY/SMH/XLK 列进停用词表：基准
是当轮取到哪张卡就是谁，而其中一只（IVV）恰好是这个用户真实持有的仓位，列表
既会漏又会误伤。

**这个数字改变了一个决定。** 之前连续三轮往停用词表加词，我判断「加词是止血
不是修复」，并提出一个更贵的结构性方案（从卡片模板反推非 ticker 词表）。
2.2% 说明那条路目前撑得住，**暂时没有理由做那次重构**——留在已知限制里，等
误报率抬头再说。这正是先测量再动手的价值：它拦下的是一次投机性的重写。
