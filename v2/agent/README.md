...
- **超预算率 21%（agent）/ 9%（routed）/ 7%（production）**，且**三次证实描述改动
  ...
  是产品决定。
- **稳定失败 1 条（r07），抖动 6 条**：抖动按定义没法凭一轮定位，3 次采样还不够。
  在能稳定复现之前不动它们 —— 这条规矩本身是第 3 轮买来的教训。
  温度已经是 0，「调低温度」这个旋钮不存在；能做的是把抖动**按分叉点分类**：
  每次运行记录它走过的工具路径（JSON 里的 `trace`）和答案原文（`answer`），
  重复采样后把失败运行和通过运行并排，看第一处不同在哪 ——
