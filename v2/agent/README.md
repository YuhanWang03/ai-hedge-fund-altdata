...
删了。修法是给重写指令补一个模型无法自知的事实——**哪些数字是对的、必须原样
保留**（之前只说哪些错了）。这一条还没量，见已知限制。
...
- **重写指令的「保留清单」还没量**：p04 两次重写丢了当日盈亏，指令现在会列出初稿里
  溯源成功的数字并要求原样保留。有测试钉住指令内容，但它是否改变模型行为要靠
  `EVAL_CASES=p04 EVAL_REPEAT=10` 来回答——第 11 轮的教训是听起来对的改动也要过秤。
