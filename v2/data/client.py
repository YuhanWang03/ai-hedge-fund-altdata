...
class ProviderRequestError(RuntimeError):
    """Typed provider failure without response bodies or credentials."""

    def __init__(self, error_type: str, endpoint: str, status_code: int | None = None) -> None:
        self.error_type = error_type
        self.endpoint = endpoint
        self.status_code = status_code
        super().__init__(f"Financial Datasets {error_type} at {endpoint}" + (f" (HTTP {status_code})" if status_code else ""))
...
class FDClient:
    ...
    def _request(
        ...
        **kwargs,
        ...
        """HTTP request with typed failures and bounded retry on 429."""
        ...
        for attempt, delay in enumerate((*self._RETRY_DELAYS, None)):
            try:
                ...
                raise ProviderRequestError(_error_type(exc), path) from exc
            except requests.ConnectionError as exc:
                raise ProviderRequestError(_error_type(exc), path) from exc
            except requests.RequestException as exc:
                raise ProviderRequestError("UNREACHABLE", path) from exc
            ...
            if resp.status_code in {401, 403}:
                raise ProviderRequestError("AUTH_ERROR", path, resp.status_code)
            if resp.status_code == 404:
                # A valid lookup for a missing ticker/resource is a data
                # availability result, not a provider outage.
                raise ProviderRequestError("EMPTY_DATA", path, resp.status_code)
            if resp.status_code == 429:
                raise ProviderRequestError("RATE_LIMITED", path, resp.status_code)
            if resp.status_code >= 400:
                raise ProviderRequestError("DEGRADED", path, resp.status_code)
        ...
        raise ProviderRequestError("RATE_LIMITED", path, 429)
