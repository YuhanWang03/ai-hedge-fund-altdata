...
def quote_event(conn, event):
    """Called within a write transaction; quota reservation and event write are atomic."""
    from v2.data.billing_rules import resolve_alias, allocate_tavily
    ...
    if event.get('state') != 'success':
        ...
        # Once configured, the account-aware plan takes precedence over a raw unit price.
        quota = conn.execute('SELECT 1 FROM tavily_quota LIMIT 1').fetchone()
        if quota or not price:
            cost, reason = allocate_tavily(conn, event)
            price = event.get('price')
        elif price.get('review_after', at) < at:
            reason = '价格已到复核期限'
        elif usage.get('units') is not None:
            cost = usage['units'] * price['rates']['unit']
            event['quota_note'] = '手动单价估算，未抵扣免费额度'
        else:
        ...
        if category == 'llm':
            ...
            if inp is not None and out is not None and (cached is not None or rates['input'] == rates['cached_input']):
                ...
                if 0 <= cached <= inp and out >= 0:
                    event['breakdown'] = {key: {'tokens': count, 'rate': rates[key], 'amount': count*rates[key]/1_000_000} for key, count in [('input', inp-cached), ('cached_input', cached), ('output', out)]}
...
def reconcile_pending():
    ...
    with _conn() as conn:
        ...
        for row in rows:
            ...
            updated += 1
    return {'updated': updated, 'message': f'已补算 {updated} 条 LLM 记录；未找到历史价格或完整用量的记录不变。Tavily 历史免费额度不倒推。'}


def report(limit=100):
    from v2.data.price_sync import sync_status
    from v2.data.billing_rules import quota_status
    ...
    return {**totals, 'currencies': list(currencies.values()), 'timezone': 'America/New_York', 'basis': 'usage_estimates',
            'by_provider': list(groups.values()), 'prices': prices(), 'price_sync': sync_status(), 'tavily_quota': quota_status(), 'pending_reasons': pending_reasons,
